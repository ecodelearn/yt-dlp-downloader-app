...
case "$CHOICE" in
    "Vídeo MP4")
        yt-dlp -f 'bv*[vcodec^=avc]+ba[acodec^=mp4a]/bv*+ba/b' --merge-output-format mp4 --remux-video mp4 "$URL" 2>&1
        ;;
    "Áudio MP3")
        yt-dlp -x --audio-format mp3 "$URL" 2>&1
        ;;
    "Vídeo com Legendas")
        yt-dlp -f 'bv*[vcodec^=avc]+ba[acodec^=mp4a]/bv*+ba/b' --merge-output-format mp4 --remux-video mp4 --write-srt --sub-lang en "$URL" 2>&1
        ;;
    "Playlist Áudio")
        yt-dlp --yes-playlist -x --audio-format mp3 "$URL" 2>&1
        ;;
    "Playlist Vídeo")
        yt-dlp --yes-playlist -f 'bv*[vcodec^=avc]+ba[acodec^=mp4a]/bv*+ba/b' --merge-output-format mp4 --remux-video mp4 "$URL" 2>&1
        ;;
esac
